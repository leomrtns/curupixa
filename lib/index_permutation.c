...
crpx_index_permutation_new (size_t n)
{
  crpx_index_permutation_t p = (crpx_index_permutation_t) malloc (sizeof (crpx_index_permutation_struct));
  p->size = n;
  p->idx = NULL;
  p->idx = (size_t *) crpx_malloc (n * sizeof (size_t));
  if (!p->idx) {
    free (p);
    return NULL;
  }
...
{
  if (i >= size) {
    crpx_log_error ("index_permutation_swap: location i = %zu out of range (larger than size %zu)", i, size);
    return false;
  }
  if (j >= size) {
    crpx_log_error ("index_permutation_swap: location j = %zu out of range (larger than size %zu)", j, size);
    return false;
  }
...
crpx_index_permutation_is_valid (const crpx_index_permutation_t *p) // just a check, therefore doesn't raise an error but logs a debug
{
  ...
  for (i = 0; i < p->size; i++) {
    if (p->idx[i] >= size) { 
      crpx_logger_debug ("index %zu at location %zu is out of range (larger than size %zu)", p->idx[i], p->size);
      return false;
    }
    for (j = 0; j < i; j++) if (p->idx[i] == p->idx[j]) {
      crpx_logger_debug ("index %zu is repeated at locations %zu and %zu", p->idx[i], i, j);
      return false;
    }
  }
  return true;
...
{ // q[] will have the elements of p[] which would make p[] the identity index (i.e. p[q[i]] = i)
  size_t i ;
  if (q->size != p->size) {
    crpx_log_error ("index_permutation_inverse: q->size = %zu, p->size = %zu", q->size, p->size);
    return false;
  }
  for (i = 0; i < size; i++) q->idx[p->idx[i]] = i;
  return true;
}
...
crpx_index_permutation_combine (crpx_index_permutation_t * p, const crpx_index_permutation_t * pa, const crpx_index_permutation_t * pb)
{
  ...
  if (pa->size != p->size) {
    crpx_log_error ("index_permutation_combine: input pa->size = %zu but output size = %zu", pa->size, p->size);
    return false;
  }
  if (pb->size != p->size) {
    crpx_log_error ("index_permutation_combine: input pb->size = %zu but output size = %zu", pb->size, p->size);
    return false;
  }
...
crpx_index_combination_new (size_t n, size_t k)
{
  crpx_index_combination_t c = (crpx_index_combination_t) malloc (sizeof (crpx_index_combination_struct));
  c->n = n;
  c->k = k;
  c->idx = NULL;
  c->idx = (size_t *) crpx_malloc (k * sizeof (size_t));
  if (!c->idx) {
    free (c);
    return NULL;
  }
...
{
  if (!c) return;
  for (size_t i = 0; i < c->k; i++) p->idx[i] = i;
}
...
{
  if (!c) return;
  for (size_t i = 0; i < c->k; i++) p->idx[i] = c->n - c->k + i;
}
